...
The 16 GB / `OLLAMA_NUM_PARALLEL=4` / `num_ctx≥8192` budget is a hard envelope. Independent measurements on the RTX 4080 confirm that Qwen3 14B Q4_K_M lands at ~12 GB resident with a 19K context (Glukhov, March 2026 benchmark using Ollama 0.17.7), and the GPU saturates at roughly 51–62 tokens/sec on 14B-class Q4 models. Any 27B+ dense model at Q4 needs ~17–19 GB and will spill to CPU at parallel=4. Two practical implications:

- **The realistic ceiling is ~14B dense at Q4 or ~20B sparse at MXFP4** to keep parallel=4. Lowering to parallel=2 unlocks Mistral Small 3.2 (24B Q4_K_M ≈ 14 GB) cleanly and 27B Q4 marginally.
...
- **Qwen3 30B-A3B Instruct-2507 / Qwen3.5-35B-A3B / Qwen3.6-35B-A3B (MoE)** — at Q4_K_M these need ~17–18 GB resident and spill on a 16 GB card. They are excellent models but do not satisfy the hard constraint with parallel=4. If you can drop to parallel=1, the Qwen3-30B-A3B-Instruct-2507 Q4 is one of the best non-thinking instruction models available and is worth a side experiment.
- **Gemma 3 27B** — same VRAM problem; 17 GB Q4 spills.
- **Llama 3.3 70B / Llama 4** — far outside 16 GB; not viable locally.
- **Command-R7B** — small (7B), grounded-generation/citation-aware, but Qwen3 14B already provides stronger reasoning. Worth knowing about for the Stage 4b `evidence_quote` field (its native grounding tags emit citation spans) but not the right primary classifier. There is no first-party Ollama Command-R7B tag — community ports exist.
...
### Specific reasoning on the open questions
...
2. **Chain-of-Thought + LLM-as-encoder** — "Chain-of-Thought Embeddings for Stance Detection" (arXiv 2310.19750). Generate a CoT explanation with the LLM, then feed the explanation text into a smaller transformer encoder for the actual classification. This recovers most of the CoT benefit while avoiding "stance label hallucination" where the LLM's correct reasoning gets paired with the wrong final label — exactly the regression mode observed when a "scan for refutation" instruction was added.
...
The takeaway for Stage 4b is direct: the article and the claim are already in hand, so steps 1–2 are effectively "extract sentences mentioning or relevant to this claim." Steps 3–4 are where the win is — *a per-passage stance pass followed by an article-level aggregation step.*
...
- **Constitutional/self-critique**: the failed "scan for refutation" prompt described earlier is in this family. Works only when the critique is structurally independent (CoVe-style), not when bolted onto the same prompt.
...
The dominant failure mode in the revised prompt was that the model treated *any* contrasting language as substantive refutation. The fix is structural, not prompt-engineered:
...
The current `nomic-embed-text` (137M, 8192 ctx) is solid for general retrieval but the recent benchmarks show it underperforming on harder retrieval tasks. April 2026 evidence:
...
- The current 90% accuracy ceiling is not the natural ceiling for this task — it is the zero-shot ceiling for Qwen3 14B *without* learned distinctions for the quote-then-refute pattern. Targeted fine-tuning is the right tool.
...
- Free Colab notebooks for **Qwen3 14B fine-tuning that fit on a Tesla T4 (16 GB)** — i.e., directly maps to a 16 GB RTX 4080.

Axolotl is the better choice for multi-GPU or YAML-config-driven workflows; Torchtune is the cleanest PyTorch-native option but more boilerplate for a first run. **Use Unsloth for the first fine-tune.** Migrate to Axolotl later if/when scale demands it.
...
- **First choice: Qwen3 14B (base, not 2507 instruct).** It is the existing production model, so the fine-tune is a true delta. Strong reasoning + instruction-following baseline. Unsloth has a maintained Colab notebook for Qwen3 14B QLoRA.
...
There are 100 labelled gold rows currently, expandable to ~500. The recommended plan:
...
No. The empirical Qwen3 quantisation study and the long-context quantisation study both show ultra-low-bit (2–3 bit) hurts linguistic and especially long-context tasks disproportionately, and this task is long-context multi-paragraph attribution. A Q4 14B fine-tune is a strictly better use of the same budget. Skip Q3/Q2 32B.
