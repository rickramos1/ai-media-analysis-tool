...
- **Gold-set baseline + current measurements (2026-05-01)**: the original 2026-04-28 gold set was rebuilt 2026-05-01 against a fresh end-to-end pipeline rerun and re-judged by claude-opus-4-7 via `pipeline/gold_set_cloud_label.py`. The 5-model bake-off (`pipeline/gold_set_bakeoff.py`) measured accuracy on the 100-row gold set:
  - **gpt-oss-safeguard:latest** (production verifier): accuracy 0.83, **carrier precision 1.000**, recall 0.542, F1 0.703 — chosen for the precision win
  - phi4:14b: 0.71, 0.652, 0.625, 0.638
  - qwen3:14b (incumbent): 0.64, 0.682, 0.625, 0.652
  - phi4-reasoning: 0.58, 0.562, 0.375, 0.450 (reasoning hurt verdict quality)
  - gemma3:12b: 0.40, 0.577, 0.625, 0.600
  - The historical 90% / 0.84 / 1.00 baseline (2026-04-28, qwen3:14b + format=schema) is no longer reproducible — same model file, same code, ~50-56% agreement with original verdicts now. Most plausible cause: Ollama internal change between then and now. The 2026-05-01 numbers are the current reference. Operational tradeoff with gpt-oss-safeguard: the carrier list is shorter (90 verdicts / 67 articles vs qwen3's 198/138 on the same Stage 4a candidates) but every flag is reliable.
