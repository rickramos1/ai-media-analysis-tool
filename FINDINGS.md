...
1. **Corpus** — 1,486 articles from MediaCloud, scraped with `trafilatura`. 1,159 passed eligibility (word count + topic-context gate).
...
4. **Stage 3 — ideology cross-reference**: promoted 17 claim-source groups to canonical (multi-ideology debunks or authoritative-solo outlets).
5. **Stage 4a — embedding retrieval** with `nomic-embed-text`: 942 articles × 85 unique claim-texts, top-K candidates per article above cosine similarity 0.65.
6. **Stage 4b — LLM verification** (`qwen3:14b`) at sim ≥ 0.68: 2,147 candidate pairs classified as `carrying`, `debunking`, `neutral_reporting`, or `irrelevant`.
...
| Project 2025 | 9 | Project 2025 claims that medication abortion drugs have complication rates four times higher than surgical abortion. |
| Robert F. Kennedy Jr. | 1 | The NIH told 'doctors and patients not to report injuries' after taking an abortion drug. |
...
- `stage4b_verdicts.json` — full LLM verdicts for all 2,147 candidate pairs
...
- **Two campaigns dominate**: Heartbeat International (47/117) and Ethics & Public Policy Center (37/117). The pipeline detects concentrated amplification, not diverse misinformation.
...
- **9 candidate pairs returned UNKNOWN** after Stage 4b. Inspect in `stage4b_verdicts.json` if needed.
