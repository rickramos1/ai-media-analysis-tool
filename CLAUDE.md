...
- **Ollama** is required for stages 1, 2, 3.5, 4a, 4b. See `docs/ollama_setup.md` for full configuration. `OLLAMA_HOST`, `OLLAMA_MODEL` (default `qwen3:14b`), `OLLAMA_EMBED_MODEL` (default `nomic-embed-text`), `OLLAMA_PARALLEL` (default 4 in pipeline scripts) all read from `.env`.
